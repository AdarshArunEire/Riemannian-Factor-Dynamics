...
- unique pointwise Fréchet means define the population centre path — **and this is now split by geometry**. On Bures–Wasserstein it is automatic for every law charging the full-rank cone (§14.2 R1, R2). On spheres and sphere products it can fail, and where it fails no continuous selection exists and any single-valued convention injects the lag-invariant contamination of §14.2. On the latent-stochastic-centre class the phrase has no referent at all (§4.1);
...
**R2 — uniqueness. Splits by geometry; the selector is the crack. PROVED.**

*On BW the gate applies.* \(F_Q\) is convex in the **ordinary linear structure** on \({\rm PSD}(m)\), despite BW being nonnegatively curved, because
...
exhibits \(G\) as an infimum of functions affine in \(A\), hence concave; the identity reduces to \(\operatorname{tr}S+\operatorname{tr}(S^{-1}N^2)-2\operatorname{tr}N=\operatorname{tr}(S^{-1}(S-N)^2)\ge0\) with equality iff \(S=N\). Strictness follows because the optimiser \(T^\star(A)=A^{-1/2}(A^{1/2}\Sigma A^{1/2})^{1/2}A^{-1/2}\) is constant along \(A_0+tH\) only if \(THT=0\), i.e. \(H=0\). Therefore the BW Fréchet mean is **unique** for every \(Q\) charging the full-rank cone. This is proved internally for arbitrary \(Q\) and strictly extends Agueh–Carlier, Theorem 6.1, which covers finitely many atoms.
...
External producers consumed by the closure package, each with exact theorem and scope in [[References and external claim audit]]: Doob's mean-square ergodic theorem (ID-3, ID-7); Gavrilov/Pennec's neighbouring-log expansion (ID-8); Dahlhaus–Richter–Wu Assumption 2.1 (ID-10); Agueh–Carlier Theorem 6.1 (ID-9 R2, superseded internally for arbitrary \(Q\)); Kuratowski–Ryll-Nardzewski measurable selection (ID-9 R2).
