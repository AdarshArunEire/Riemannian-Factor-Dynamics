...
1. Each frozen marginal Fréchet objective has a nonempty singleton argmin, or the paper explicitly changes its target to a minimizer set/selected centre. Every Log uses one declared branch on the almost-sure support. On Bures–Wasserstein this is automatic: the Fréchet mean of any law charging the full-rank cone exists in the open cone and is unique. On spheres and sphere products it can fail, and where the argmin is nonsingleton **no continuous selection exists**; a selector jump of size \(\Delta\) at sample fraction \(\lambda\) injects a rank-one, lag-invariant, bandwidth-insensitive contamination \(\lambda(1-\lambda)\Delta^2\) into **every** lag of the row, which dominates an AR(1) factor at every \(h\ge1\) once \(\lambda(1-\lambda)>\rho/4\).
