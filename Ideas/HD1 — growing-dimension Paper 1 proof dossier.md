...
There is no OPEN or CONDITIONAL node in the dependency graph of HD-E. Later application-map work proved a broader causal Hilbert physical-dependence version of the robust chain, fixed-band higher AIRM differentials, and two sharper oracle branches. The first uses exact sample separation, lag-specific GLO, and a negligible direct non-rigid frame coefficient, with one exact flat as the clean sufficient geometry. The second is FRAME-2P-U: an entirely observable three-colour, undersmoothed two-path polygon correction with \(b_n=n^{-1/7}\), \(M_n\asymp n^{2/7}\), and \(c_n=n^{-\gamma}\), \(1/6<\gamma<3/14\). Under its explicit dimension-uniform generated-tube, composed-action, replacement, mask, dependence, GLO, and included-lag producers, its post-influence mean/frame remainder is \(o_p(n^{-1/2})\) and its row is root-\(n\) for arbitrary \(p_n\). Bounded total energy alone does not supply that package. These sharper branches do not alter HD-E. Infinite-memory conditional splitting remains open. The later HE campaigns separately rederived the mean/frame and lag-row inputs with \(R_n\) under bounded-tail and expanding-domain truncation packages; those results are not corollaries of HD-E. The adjudicated local BW analogue supplies the geometric inputs at fixed margin and under a restricted fractional-normal shrinking regime; global, rank-changing, pervasive shrinking-normal, and unrestricted sharp-window variants remain outside that theorem class.
