...
11. P1-ID: the information-set hierarchy, unique-mean law functional, flat minimum-dynamic-rank quotient, Gaussian-FDD quotient, pointwise spectral recoverability and uniform impossibility boundary, exact curved compatible-chart orbit with rank-inflation counterexample, fixed-centre nine-term contamination identity with curved remainder, and the protected Factor 1 interpretation.
...
Canonical theorem boundaries are [[P1-ID — centre-drift and factor identification boundary]], [[HE — canonical growing-energy theorem boundary]], [[BW-FIXED-MARGIN — canonical local and size-uniform theorem boundary]], [[BW-SHRINKING-MARGIN — canonical restricted theorem boundary]], [[FRAME-2P-U — conditional two-path debiasing theorem]], and [[P1-LOSS — forecast-evaluation geometry and proxy-robustness boundary]]. Archived campaign dossiers are proof provenance only.
...
- **APP-MONTHLY-VAR first:** use the existing 240-month APP-FIN panel, months
  ...
  one-step forecasts. Hold \(r=2\) and lag horizon \(h=6\) fixed. Parent RFM
  and RFD must share the same projected-score OLS VAR(1), forecast origins,
  inputs, and losses;
- preserve the parent's published centre policy: estimate its global centre
  once on the initial training window and hold it fixed. RFD must use an
  expanding-prefix path available at the origin and a declared one-month
  future-centre rule; the initial bridge carries its one-sided terminal centre
  forward;
...
- authoritative observation contract, competitors, stop rules, and non-claims:
  [[Home application — hourly crypto realised covariance]].
...
7. **APP-MONTHLY-VAR is complete.** Preserve the uncomfortable 36-origin
   ...
   decodes, minimum forecast eigenvalue \(4.16\times10^{-6}\), maximum
...
Sections 3–5 are the only live queue. Archived P1-ID, HE, BW, FRAME-DB, and FRAME-IF dossiers are proof provenance, not parallel status sources; their canonical theorem boundaries carry producer classification and current status. P1-ID is closed as an identification and interpretation theorem, and no live queue item reopens it.
