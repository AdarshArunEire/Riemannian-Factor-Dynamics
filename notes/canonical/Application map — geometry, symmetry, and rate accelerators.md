...
| Drift-amplitude threshold | state the centre family, intrinsic path motion, orientation, cross/base-change regime, estimator pair, tuning, target, horizon and admissible risk; verify whether the advantage set is actually single-crossing | report the complete risk-difference curve, not a universal \(\nu^\star\). The \(n^{-3/14}\) candidate applies only when the first surviving row defect is quadratic in \(\nu\); see P1-ID §17 |
...
### 0A.3 The \(\nu\)-phase diagram is property-first

Use \(\mu_\nu(u)=\operatorname{Exp}_{\mu_0}\{\nu g(u)V\}\) as a controlled simulation family, but report intrinsic path length/energy alongside \(\nu\). Before matching an application, determine whether the leading fixed-centre defect is linear or quadratic in \(\nu\), whether drift is aligned, orthogonal or partial relative to the identified dynamic span, and whether curvature activates ID-8. Only after that classification should the experiment compare fixed- and moving-centre risks. Structural loading recovery and forecasting are distinct targets and may cross at different points; a forecast comparison must obey P1-LOSS.
