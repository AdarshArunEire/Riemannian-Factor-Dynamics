...
Richardson centre extraction remains open to replacement in this follow-up. Its signed cancellation is elegant but can amplify variance, and its finite-sample performance depends on bandwidth, curvature, noise, and local sample availability. After establishing the forecasting layer on real or realistic data, compare it with a learned **causal** centre head using identical masks and untouched future blocks. The learned head must preserve SPD/generated-domain validity and smoothness and must be hostile-tested on P1-ID-equivalent laws; apparent recovery of an impossible centre/factor split is leakage or an undeclared convention.
