...
$runner = Join-Path $repoRoot 'experiments\run_appfin_score_filter.py'
$outputName = if ($SmokeMonths -gt 0) { 'appfin_score_filter_smoke' } else { 'appfin_score_filter' }
$outputDirectory = Join-Path $repoRoot "results\intermediate\$outputName"
$logDirectory = Join-Path $repoRoot 'results\intermediate\appfin_score_filter_runner'
$stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$stdoutPath = Join-Path $logDirectory "appfin_heads_$($stamp)_stdout.log"
$stderrPath = Join-Path $logDirectory "appfin_heads_$($stamp)_stderr.log"
...
Write-Host '1/2 APP-FIN score-head focused tests'
& $python -m pytest py/tests/test_appfin_score_filter.py py/tests/test_score_filter.py -q
...
Write-Host 'Running APP-FIN RFM/RFD × VAR/KF replay.' -ForegroundColor Cyan
Write-Host 'The parent stage and every RFD origin are digest-checked and resumable.'
if ($SmokeMonths -gt 0) {
    Write-Host "Smoke scope: first $SmokeMonths forecast month(s)."
} else {
    Write-Host 'Full scope: 36 causal expanding-origin forecasts.'
}
Write-Host "Started: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
try {
    if (-not $process.Start()) { throw 'APP-FIN score-head process could not be started' }
    ...
    while (-not $process.WaitForExit($HeartbeatSeconds * 1000)) {
        ...
        $parentDone = Test-Path -LiteralPath (Join-Path $outputDirectory 'parent_representations\metadata.json') -PathType Leaf
        $live = @(Get-Process -Id $process.Id -ErrorAction SilentlyContinue)
        $ram = [math]::Round(($live | Measure-Object WorkingSet64 -Sum).Sum / 1MB, 1)
        Write-Host (
            '[heartbeat {0}] APP-FIN score heads | elapsed={1:hh\:mm\:ss} | parent={2} | RFD origins={3} | RAM={4}MB' -f
            (Get-Date -Format 'HH:mm:ss'), $timer.Elapsed, $parentDone, $completed, $ram
    ...
    if ($process.ExitCode -ne 0) {
        throw "APP-FIN score-head run failed with exit code $($process.ExitCode)"
    }
}
finally {
    $timer.Stop()
...
Write-Host "Plots:  $outputDirectory"
